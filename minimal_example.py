...
# +
test = np.random.random((25,25)) + 1j * np.random.random((25,25))
test += test.conjugate().transpose()

eigs, vecs = scipy.linalg.eigh(test)
# -

np.allclose(eigs, np.diag(vecs.conjugate().transpose() @ test @ vecs))

np.allclose(test, vecs @ np.diag(eigs) @ vecs.conjugate().transpose())
