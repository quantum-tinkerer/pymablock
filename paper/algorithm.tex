...
\begin{enumerate}
  \item The unperturbed Hamiltonian $H_0$ is diagonal: $H_0 = (H_0)_{D}$.
  \item The offdiagonal part of any operator has no matrix elements within eigensubspaces of $H_0$.
  \item The diagonal and offdiagonal parts of an operator add to the operator itself: $\mathcal{A} = \mathcal{A}_{D} + \mathcal{A}_{O}$.
\end{enumerate}
...
When applied to the Schrieffer--Wolff transformation, the unperturbed Hamiltonian is a $2\times 2$ block matrix
\begin{equation}
H_0 = \begin{pmatrix}
H_0^{AA} & 0 \\
0 & H_0^{BB}
\end{pmatrix},
\end{equation}
and the offdiagonal part of an operator are its two offdiagonal blocks.
All the series here may be multivariate, and they represent sums of the form
%
\begin{equation}
