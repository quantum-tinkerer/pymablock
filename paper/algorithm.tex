...
A common approach to constructing effective Hamiltonians is to use the
...
matrix $\mathcal{\rho}$ of a perturbed system as a power series with respect to a
...
$\mathcal{\rho}^2 = \mathcal{\rho}$ and $[\mathcal{H}, \mathcal{\rho}]=0$, which avoid
...
Because Eq.~\eqref{eq:H_tilde_def} contains $H_0$ multiplied by
