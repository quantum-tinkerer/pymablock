...
\end{equation}
%
Repeating this process for the states $\ket{1_{t}0_{r}}$, $\ket{0_{t}1_{r}}$, and $\ket{1_{t}1_{r}}$ requires requesting \mintinline{python}|H_tilde[1, 1, 2][0, 0]|, \mintinline{python}|H_tilde[2, 2, 2][0, 0]|, and \mintinline{python}|H_tilde[3, 3, 2][0, 0]|, and yields the desired resonator frequency shift:
%
\begin{equation}
\begin{aligned}
...
This is an alternative to separating a Hamiltonian into blocks, and its only requirement is that any two states to decouple are not degenerate, unless the perturbation does not couple them in the first place.
...
We apply the mask to the Hamiltonian by providing it as the \mintinline{python}|fully_diagonalize| argument to \mintinline{python}|block_diagonalize|\footnote{The full code is available at \url{https://pymablock.readthedocs.io/en/latest/tutorial/getting_started.html}.}.
%
\begin{minted}{python}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{figures/selective_diagonalization.pdf}
    \caption{
