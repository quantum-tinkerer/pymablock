...
To design Pymablock's algorithm, we consider scenarios for which an efficient
procedure is needed, of which we identify two categories: problems in which one
wants to find an interpretable effective model, and problems in which one needs
to sample the value of an observable that is expensive to compute.
The study of superconducting qubits, for example, lies in the first category.
In this case, the Hamiltonian is constructed from several bosonic operators, so
its Hilbert space is infinite-dimensional, and the coupling between bosons
makes the Hamiltonian hard to diagonalize.
The qubits, however, are described by a few low energy states, and obtaining an
analytical effective model is essential to interpreting their behavior,
because the model provides a clear dependence of dispersive shifts, effective
couplings, and parasitic crosstalk couplings on the circuit's
parameters~\cite{Zhu_2013, Li_2020, Blais_2021, Sete_2021}.
The same holds for circuit QED systems overall, for which computational tools
are being actively developed~\cite{Groszkowski_2021, Chitta_2022, Li_2022}.
In the second category, we find materials and large device simulations, like
tight-binding models of twisted bilayer graphene~\cite{Bernevig_2021} for which
a k.p model is used, Majorana devices~\cite{Melo_2023}, and quantum dot
arrays.
In these cases, we may no longer truncate the Hilbert space, and finding the
ground state amounts to solving a sparse eigenvalue problem, which quickly
becomes expensive due to the extensive number of sites and orbitals.
...
purpose of illustrating the package's capabilities with symbolic Hamiltonians,
...
system over a square lattice of $L \times W = 200 \times 40$ sites, with 2
orbitals per unit cell~\footnote{The full code is available at
...
as shown in Fig.~\ref{fig:QD_spectrum}.
...
We consider two perturbations: the barrier strength between the quantum dots
and the superconductor, $t_{\text{barrier}}$, and the asymmetry of the dots
potentials, $\delta \mu$.

The system is large: with this many sites even storing all the eigenvectors
would take $60$ GB of memory.
Therefore, we use sparse matrices and compute only a few eigenvectors, because
perturbation theory allows us to compute the effective Hamiltonian of the low
energy degrees of freedom only.
To get the unperturbed Hamiltonian, we use the values $\mu_n=0.05$,
$\mu_{sc}=0.3$, $\Delta=0.05$, $t=1.0$, and $t_{\text{barrier}}=0$.
The barrier strength and the asymmetry of the dot potentials are the two
perturbations whose values we vary.
Each of these is a sparse array of size $63042 \times 63042$, with $333680$
non-zero elements for the unperturbed Hamiltonian, and $632$, and $126084$
non-zero elements for the perturbations, respectively.
The structure of the Hamiltonian is shown in the left panel of
Fig.~\ref{fig:QD_spectrum}, where we use a smaller system for visualization.
...
%
\inputminted[firstline=68, lastline=70]{ipython}{code_figures/lattice_system.py}
%
...
perturbation.
%
\inputminted[firstline=73, lastline=78]{ipython}{code_figures/lattice_system.py}
%
