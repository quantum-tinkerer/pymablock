...
from .second_quantization_helpers import occupation_matrices


@pytest.mark.parametrize("num_modes", range(4))
def test_adjoint_matches_occupation_matrices(num_modes):
    """Check every monomial and fermion/spin assignment through three modes."""
    identity = np.eye(1 << num_modes)
    for fermions in range(1 << num_modes):
        operators = [
            (FermionOp if fermions & (1 << i) else SigmaMinus)(f"a{i}")
        ...
        for operator in operators:
            ...
            local_factors.append((identity, lowering, lowering.T, lowering.T @ lowering))

        monomials = []
        for code in range(1 << (2 * num_modes)):
            matrix = identity
            for mode, factors in enumerate(local_factors):
                local = ((code >> mode) & 1) + 2 * ((code >> (mode + num_modes)) & 1)
                matrix = matrix @ factors[local]
            monomials.append(matrix)

        for code, matrix in enumerate(monomials):
            adjoint, sign = adjoint_monomial(
                code, num_modes=num_modes, anticommuting_modes=fermions
            )
            np.testing.assert_array_equal(sign * monomials[adjoint], matrix.conj().T)
...
def test_validation() -> None:
    assert adjoint_monomial(0, num_modes=0) == (0, 1)
    with pytest.raises(ValueError, match="within the binary basis"):
        adjoint_monomial(0, num_modes=2, anticommuting_modes=0b100)
