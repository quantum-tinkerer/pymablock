...
def adjoint_monomial(
    ...
    *,
...
) -> tuple[PackedMonomial, int]:
    """Return the adjoint monomial and its fermionic sign."""
    creators, numbers, annihilators = masks_from_monomial(code, num_modes=num_modes)
    active_modes = _mode_mask(num_modes)
    ...
    # Reversing k odd fermionic factors makes k*(k-1)/2 exchanges.
    # Number factors are even, and spin factors commute across modes.
    odd_factors = ((creators | annihilators) & anticommuting_modes).bit_count()
    sign = -1 if (odd_factors * (odd_factors - 1) // 2) % 2 else 1
    return _pack_masks(creators | numbers, annihilators | numbers, num_modes), sign
