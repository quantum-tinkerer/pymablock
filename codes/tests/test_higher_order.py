...
def test_simple_model():
    ...
    mat02 = np.diag(ev2)
    mat12 = {sympy.Symbol('x'): np.array([[0, 0, 0, 1], [0, 0, 1, 0], [0, 1, 2, 0], [1, 0, 0, 2]])}
    evec2 = np.eye(4)
    indices2 = [0, 1]
    ...
    # Test KPM method with precalculated moments
    kpm_params = dict(num_moments=1000)
    evec_A = evec2[:, indices2]
    evec_B = None

    model = get_effective_model(mat02, mat12, evec_A, evec_B=evec_B, order=8,
                                kpm_params=kpm_params, _precalculate_moments=True)
