...
`astro-myst-notebooks-0.2.1.tgz` is built from the sibling repository at
...
Current artifact source: commit `ef3e26b` in that repository.

After changing the integration, repack it and replace this artifact. Run
`npm install ./vendor/astro-myst-notebooks-0.2.1.tgz` from docs/starlight to update
