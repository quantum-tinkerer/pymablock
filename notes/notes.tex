...
We observe that the expressions \eqref{eq:lowdin-get-s} for $Y$ can be expanded in terms of the restricted operators (i.e. $H'_{AA}$, $H'_{AB}$, etc.).
...
\begin{equation}
\left(S^{(n)}_{AB}\right)_{mi} =\left(Y^{(n)}_{AB}\right)_{mj} \left(\frac{1}{E_m - H_0}\right)_{ji} = \left[G(E_m, H_0)_{ij} \left(Y^{(n)}_{AB}\right)_{mj}^* \right]^* .
\end{equation}
...
We obtain this estimate by the following reasoning: A single evaluation of the KPM Green's function on a vector costs $O(NM)$. To get $S^{(j)}$, we need to apply $G$ to $a j$ vectors on the right hand side, as $Y^{(j)}$ is a polynomial of the small parameter.
We argue that the KPM step is the costliest part of the procedure.
Evaluation of $Y$ and $\tilde{H}$ only involves products of small or sparse matrices.
There is, however, a rapidly growing combinatorical prefactor in the number of terms involved in these expressions.
These factors are even larger if there are multiple small parameters in the expansion.
We defer in-depth analysis of the complexity for now.
